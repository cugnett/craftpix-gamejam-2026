...
func _display_player_health() -> void:
	for i in range(0,player_health):
		var hearth_inst = hearth.instantiate()
		hearth_inst.position = life_position + i * Vector2(16,0) # inst.get_node("Sprite2D").texture.region.w
		add_child.call_deferred(hearth_inst)
